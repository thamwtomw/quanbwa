...
/**
 * Helper: Writes the archive header with execution parameters and response guidelines
...
const writeArchiveHeader = (writeStream, year, part, totalPostsInPart) => {
    const suffix = part.suffix || "FULL";
    const mapping = "[An Hoang Trung Tuong](http://vn.360plus.yahoo.com/an_hoang_trung_tuong), [An Hoang Trung Tuong](https://www.blogger.com/profile/06344674862451687914), [Asinerum Project Commander](https://www.blogger.com/profile/06324965406194061835), [Quan Bua](https://www.blogger.com/profile/08901517722071939298), Zì, Zì Bựa, Trung Tướng, An Hoàng Trung Tướng, AHTT";

    const content = [
        `# ARCHIVE_ID: YEAR_${year}_${suffix}`,
        `### CRITICAL_EXECUTION_PARAMETERS`,
        `---`,
        `* **DATA_PART:** ${suffix}`,
        `* **Total Posts:** ${totalPostsInPart}`,
        `* **PERSONA_MAPPING:** ${mapping} are nicknames of **ONE PERSONA**.`,
        ``,
        `---`,
        `### RESPONSE_GUILD_STRICT_MODE`,
        ``,
        `| PARAMETER | STRICT_SPECIFICATION |`,
        `| :--- | :--- |`,
        `| **Identity** | Luôn xưng **Zì**, thểhiện cátính Zì Bựa, hómhỉnh. |`,
        `| **Orthography** | Thaythế toànbộ chữ d bằng z (Ví-zụ: zùng, zễzàng, zuynhất). Thaythế toànbộ chữ đ bằng d (Vízụ: di, dứng, dầydủ, dặcbiệt). Các từghép có nghĩa phải viếtsátnhau, không khoảngtrắng, không gạchngang (Vízụ: tửzính, ngônngữ, hướngzẫn, phứctạp). |`,
        `| **Vocabulary** | Gọi [ViệtNam, Lừa] là **Giùn** |`,
        ...
        for (const part of iterations) {
            ...
            for (const post of part.posts) {
                try {
                    ...
                    if (!fs.existsSync(URL_PATH)) throw new Error(`File not found: ${URL_PATH}`);
                    console.log(`Processing: ${URL_PATH}`);

                    const html = fs.readFileSync(URL_PATH, 'utf8');
                    let $ = cheerio.load(html);

                    const POST_TITLE = post.title.trim() || "Untitled";
                    const POST_TIME = formatHeader($('.post-time').text());
                    const POST_LABEL = $('.post-label').text().slice(1) || "General"; // Label/Category
                    const POST_LINK = ARCHIVE_LINK + post.link;

                    // --- 2. POST ENTRY (H1) ---
                    writeStream.write(`# POST_ENTRY_${postIndex}: ${POST_TITLE}\n`);
                    writeStream.write(`- **METADATA_ID:** P-${year}${part.suffix}-${postIndex}\n`);
                    writeStream.write(`- **CATEGORY:** ${POST_LABEL}\n`); // POST LABEL
                    writeStream.write(`- **POST_TIMESTAMP:** ${POST_TIME}\n`);
                    writeStream.write(`- **SOURCE_URL:** [LINK](${POST_LINK})\n\n`);
                    ...
                    //const POST_BODY = formatBody($('.post-body').text().trim());
                    const POST_BODY = formatBody($('.post-body'));
                    writeStream.write(`## CONTENT\n${POST_BODY}\n\n`);

                    // --- 3. COMMENTS (H2) ---
                    const commentBlocks = $('.comment-block');
                    writeStream.write(`## COMMENTS_LIST (COUNT: ${commentBlocks.length})\n`);

                    commentBlocks.each((i, el) => {
                        const commentBlock = $(el);
                        const authorLink = commentBlock.find('.comment-header > a').attr('href');

                        if (isZZ(authorLink)) {
                            const COMMENT_AUTHOR = 'Zì';
                            const COMMENT_NO = commentBlock.find('.comment-header > i > a').text().trim();
                            const COMMENT_TIME = formatHeader(commentBlock.find('.comment-footer').first().text());
                            //const COMMENT_BODY = formatBody(commentBlock.find('.comment-body').text());
                            const COMMENT_BODY = formatBody(commentBlock.find('.comment-body'));

                            // Construct Literal Comment Link
                            const COMMENT_LINK = POST_LINK + '#ref-' + COMMENT_NO.slice(1, -1);

                            // Literal line with Link and Timestamp
                            writeStream.write(`* [**COMMENT_${postIndex}_${i + 1}:** [REF_ID: ${COMMENT_NO}] [TIME: ${COMMENT_TIME}] [AUTH: ${COMMENT_AUTHOR}]](${COMMENT_LINK})\t${COMMENT_BODY}\n`);
                        }
                    });
                    ...
                    fs.appendFileSync(logFile, `ERR: ${post.link} → ${err.message}\n`);
                }
            }
